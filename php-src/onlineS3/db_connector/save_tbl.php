...
function save_tbl($conn, $tbl_data) {
	...
	for ($i=0; $i<count($tbl_data); $i++) {
		...
		$unit = $tbl_data[$i][3];
		$rd_value = $tbl_data[$i][4];
		
		$sql = "SELECT reg_id FROM rp_region WHERE rg_label = '$region_id';";
		$res = exec_select($conn,$sql);
		$reg_id = $res[0]['reg_id'];
		
		$sql_insert = "INSERT INTO re_tables (table_id, reg_id, var_id, year_id, unit, rd_value) 
						VALUES ($tbl_id, '$reg_id', '$var_id', '$year_id', '$unit', '$rd_value')";
