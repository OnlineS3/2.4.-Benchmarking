...
<?php 
	function draw_results($conn, $group_type, $tbl_id, $show_graph, $graph_type, $src) {
		if ($group_type == 'reg') {
			$sql = "SELECT DISTINCT rt.reg_id as reg_id, rr.rg_label as label FROM re_tables rt JOIN rp_region rr ON rr.reg_id = rt.reg_id WHERE rt.table_id = $tbl_id ORDER BY rt.reg_id";
			$col = 'reg_id';
		} else {
			$sql = "SELECT DISTINCT rt.var_id as var_id, rv.var_id as label FROM re_tables rt JOIN rp_variable rv ON rv.var_id = rt.var_id WHERE table_id = $tbl_id ORDER BY var_id";
			$col = 'var_id';
		}
	...
	function draw_results_reg($conn, $group_type, $tbl_id, $region_id, $src, $show_graph) {
		...
		$sql = "SELECT DISTINCT rt.reg_id as reg_id, rr.rg_label as rg_label FROM rp_region rr JOIN re_tables rt ON rt.table_id = $tbl_id AND rt.reg_id = rr.reg_id WHERE rt.reg_id = '$region_id' ORDER BY rt.reg_id";
		$region = exec_select($conn, $sql);
		$reg_id = $region[0]['reg_id'];
		$rg_label = $region[0]['rg_label'];
		$wd_html = "<p>$rg_label - $reg_id</p><table>";
		...
		echo "<p class='section-header'>$rg_label - <span style='color:red'>$reg_id</span></p>";
		
		// get unique vars by region
		$sql = "SELECT DISTINCT var_id FROM re_tables WHERE table_id = $tbl_id and reg_id = '$reg_id' and rd_value > 0";
		...
		for($y=0; $y<count($vars); $y++) {
			...
			for($z=0; $z<count($years); $z++) {
				$year = $years[$z]['year_id'];
				$sql = "SELECT rd_value FROM re_tables WHERE table_id = $tbl_id and var_id = '$var' AND year_id = '$year' AND reg_id = '$reg_id'";
				$val = exec_select($conn, $sql);
				$value = ($val[0]['rd_value']>0) ? number_format($val[0]['rd_value'],2,".","") : 0;	
	...
	function draw_results_var($conn, $group_type, $tbl_id, $var_id, $src, $show_graph) {
		$sql = "SELECT DISTINCT rt.var_id as var_id, rv.var_descr as var_descr FROM rp_variable rv JOIN re_tables rt ON rt.table_id = $tbl_id AND 
				rt.var_id = rv.var_id AND rv.var_id = '$var_id' ORDER BY rv.var_id";
		$var = exec_select($conn,$sql);
		
		$var_id = $var[0]['var_id'];
		$var_descr = $var[0]['var_descr'];
		
		$wd_html = "<p>$var_id - $var_descr</p><table>";
		
		// get all years
		$sql = "SELECT DISTINCT year_id FROM re_tables WHERE table_id = $tbl_id AND var_id = '$var_id' ORDER BY year_id";
		$years = exec_select($conn,$sql);
		
		echo "<p class='section-header'>$var_descr - $var_id</p>";
		
		// get unique vars by region
		$sql = "SELECT DISTINCT rr.reg_id, rr.rg_label FROM re_tables re JOIN rp_region rr ON re.reg_id = rr.reg_id WHERE re.table_id = $tbl_id and re.var_id = '$var_id'";
		...
		for($y=0; $y<count($regions); $y++) {
			...
			$has_res = exec_select($conn,"SELECT 1 FROM re_tables WHERE table_id = $tbl_id AND reg_id = '$region' AND var_id = '$var_id' AND rd_value > 0");
			if (count($has_res) == 0 ) { continue;}
			$wd_html .= "<tr>";
			$region_desc = $regions[$y]['reg_id'] . ' - ' . $regions[$y]['rg_label'];
			echo "<div class='row tbl-body'>";
			...
			for($z=0; $z<count($years); $z++) {
				$year = $years[$z]['year_id'];
				$sql = "SELECT rd_value FROM re_tables WHERE table_id = $tbl_id and reg_id = '$region' AND year_id = '$year' and var_id = '$var_id'";
				$val = exec_select($conn,$sql);
				$value = ($val[0]['rd_value']>0) ? number_format($val[0]['rd_value'],2,".","") : 0;
